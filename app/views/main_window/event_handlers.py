...
def on_session_finished(self, session_type):
    ...
    try:
        ...
        print(f"Pomodoro session finished. Showing break dialog for {next_session_type}")
    ...
    try:
        ...
        try:
            if hasattr(self, 'trayIcon') and self.trayIcon:
                ...
                # 트레이 메뉴 제거
                try:
                    if hasattr(self.trayIcon, 'contextMenu') and self.trayIcon.contextMenu():
        ...
        try:
            ...
            def exit_handler():
                try:
                    with open(log_file, "a", encoding="utf-8") as f:
                        ...
                        f.write(f"{timestamp} - 핸들러 실행 시 활성 스레드 수: {threading.active_count()}\n")
                except:
                    pass
            ...
            try:
                def force_exit():
                    try:
                        with open(log_file, "a", encoding="utf-8") as f:
                            f.write(f"{time.strftime('%Y-%m-%d %H:%M:%S')} - 강제 종료 시도 (os._exit)\n")
                    except:
                        pass
                    os._exit(0)
