...
from PyQt6.QtWidgets import (QDialog, QVBoxLayout, QHBoxLayout, QLabel, 
                             ...
                             QLineEdit, QComboBox, QDateEdit, QHeaderView,
                             QWidget, QMessageBox, QCheckBox, QGroupBox,
                             QFormLayout, QToolBar, QApplication)
from PyQt6.QtCore import Qt, pyqtSlot, pyqtSignal, QDate
...
class LogDialog(QDialog):
    ...
    def setupUI(self):
        ...
        self.logTable.setHorizontalHeaderLabels(["날짜", "시간", "내용", "태그", "카테고리", "소요 시간"])
        self.logTable.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeMode.ResizeToContents)
        self.logTable.horizontalHeader().setSectionResizeMode(2, QHeaderView.ResizeMode.Stretch)  # 내용 컬럼 늘리기
    ...
    def loadLogs(self):
        ...
        for log in logs:
            ...
            # 날짜 및 시간
            date_str = log.start_date.split(' ')[0] if ' ' in log.start_date else log.start_date
            ...
            # 로그 데이터 저장
            for col in range(6):
                self.logTable.item(row_position, col).setData(Qt.ItemDataRole.UserRole, log)
    ...
    def onEditLog(self):
        ...
        QMessageBox.information(self, "로그 편집", f"로그 편집 기능은 개발 중입니다.\n선택된 로그: {log.message}")
