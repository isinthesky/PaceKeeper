...
from PyQt6.QtWidgets import QApplication, QDialog, QWidget, QPushButton, QLineEdit
from PyQt6.QtWidgets import QLabel, QListWidget, QComboBox, QFrame
import os
...
def set_object_names(dialog):
    ...
    for child in dialog.findChildren(QObject):
        ...
        if isinstance(child, QPushButton):
            ...
            elif "right" in child.objectName().lower() or child.x() >= dialog.width() / 2:
                child.setObjectName("rightPanel")
            elif "top" in child.objectName().lower() or child.y() < dialog.height() / 2:
                child.setObjectName("topArea")
            elif "bottom" in child.objectName().lower() or child.y() >= dialog.height() / 2:
...
def apply_theme_change(dialog, theme_name, theme_manager):
    ...
    if theme_manager:
        ...
        if style_content:
            ...
            # 모든 자식 위젯의 개별 스타일시트 제거
            for widget in dialog.findChildren(QWidget):
                widget.setStyleSheet("")
                
            # 특수 처리가 필요한 위젯 업데이트
            for frame in dialog.findChildren(QFrame):
                if "colorPreview" in frame.objectName() and hasattr(dialog, 'current_category'):
                    if dialog.current_category and hasattr(dialog.current_category, 'color'):
                        frame.setStyleSheet(
