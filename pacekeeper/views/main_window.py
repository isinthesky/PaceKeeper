...
class MainWindow(QMainWindow):
    ...
    def __init__(self, main_controller: Any | None, config_controller: ConfigController) -> None:
        super().__init__()

        self.main_ctrl: Any | None = main_controller
        self.config_ctrl: ConfigController = config_controller
        self.lang_res: dict[str, str] = load_language_resource(config_controller.get_language())
    ...
    def init_ui(self) -> None:
        ...
        # 태그 버튼 패널
        self.tag_buttons_panel = TagButtonsPanel(central_widget, self.on_tag_selected)
        main_layout.addWidget(self.tag_buttons_panel)

        # 텍스트 입력 패널
        self.text_input_panel = TextInputPanel(
            central_widget, self.lang_res.messages["TASK_DESCRIPTION"], self.on_text_changed
        )
        main_layout.addWidget(self.text_input_panel)

        # 최근 로그 컨트롤
        self.recent_logs_control = RecentLogsControl(
            central_widget, self.config_ctrl, self.on_log_double_clicked
        )
    ...
    def on_about(self) -> None:
        """정보 메뉴 클릭 이벤트"""
        QMessageBox.about(self, "PaceKeeper", self.lang_res.messages["ABOUT_TEXT"])

    def on_category(self) -> None:
        """카테고리 메뉴 클릭 이벤트"""
    ...
    def on_log_double_clicked(self, index: QModelIndex) -> None:
        """로그 항목 더블 클릭 이벤트"""
    ...
    def show_break_dialog(
        self, break_type: str, duration: int, on_break_end: Callable[[], None]
    ) -> None:
        """휴식 다이얼로그 표시"""
        break_dialog = BreakDialog(self, self.main_ctrl, self.config_ctrl, duration, on_break_end)
        break_dialog.show()
    ...
    def on_shortcut_start(self) -> None:
        """시작 단축키 이벤트"""
        if self.start_button.isEnabled():
            self.on_start()

    def on_shortcut_pause(self) -> None:
        """일시정지/재개 단축키 이벤트"""
        if self.pause_button.isEnabled():
            self.on_pause()

    def on_shortcut_stop(self) -> None:
        """중지 단축키 이벤트"""
